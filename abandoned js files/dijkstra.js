class Dijkstra extends GridPathFinder{
    ...
    while(this.queue.length){  // while there are still nodes left to visit
			...
			if(this.current_node_YX[0]==this.goal[0] && this.current_node_YX[1]==this.goal[1]){  // found the goal & exits the loop
				this.states.push([this.current_node, this.queue, this.visited, this.neighbours]); 
				found = true;
				console.log("found");
      ...
      for(var i=0;i<this.num_neighbours;++i){
			...
			this.states.push([this.current_node, this.queue, this.visited, this.neighbours]);
      // [1 node, array of queue, 2d array of current visited points, valid neighbours array]
		}
		if(found){
  ...
  all_state(){
		// this method displays the all path & visited nodes on the map
